	...
	<div class="row">
		...
		<div class="entry_details">
			...
			{/if}
				...
				{translate key="plugins.generic.ompcitation.link"}<a href={$publication->getId()}> {$smarty.server.HTTP_HOST}{$smarty.server.REQUEST_URI}</a> .
			...
			<div id="diviso" style="display:none;">
				...
				{/if}
					...
					DOI: <a href="{$doiUrl}">{$doiUrl}</a>
					[{translate key="plugins.generic.ompcitation.date"}{$smarty.now|date_format:"%e %B. %Y"}.]
					{translate key="plugins.generic.ompcitation.link"}<a href={$publication->getId()}> {$smarty.server.HTTP_HOST}{$smarty.server.REQUEST_URI}</a> </p>
				</div>
			</div>
			<script>
